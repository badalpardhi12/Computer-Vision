...
def savePick():
    ...
    data = {}
    data["pick"] = pick
    with open('pittsburgh-door.json', 'w') as outfile:
...
def combine():
    ...
    src_pnts_new = np.empty([4,2], np.float32)
    dst_pnts_new = np.empty([4,2], np.float32)
    for i in range(4):
        src_pnts_new[i][0] = float(pick[2][i][0])
        src_pnts_new[i][1] = float(pick[2][i][1])
        dst_pnts_new[i][0] = float(pick[3][i][0]+w)
        dst_pnts_new[i][1] = float(pick[3][i][1]+h)
    M = cv2.getPerspectiveTransform(src_pnts_new, dst_pnts_new)
    ...
    cv2.imwrite("result_wall.jpg", result)
    ...
    if len(pick[idx]) >= 4:
        ...
        if i == 'y' or i == 'Y':
            if idx >= 3:
                ...
                print('center 4 points for right')
                ...
                print('center 4 points for left')
...
#coloring points of marking
# for idx in range(len(pick)):    
#     for i in range(len(pick[1])):
#         blue = (255, 0, 0)
#         red = (0, 0, 255)
#         if (idx == 2):
#             imageL = cv2.circle(imageL, tuple(pick[idx][i]), 10, blue, 4)
#             imageL = cv2.putText(imageL, str(i), (pick[idx][i][0]+10, pick[idx][i][1]-10),
#                               cv2.FONT_HERSHEY_SIMPLEX,1, blue, 1)
#         if (idx == 3):
#             imageC = cv2.circle(imageC, tuple(pick[idx][i]), 10, blue, 4)
#             imageC = cv2.putText(imageC, str(i), (pick[idx][i][0]+10, pick[idx][i][1]-10),
#                               cv2.FONT_HERSHEY_SIMPLEX,1, blue, 1)

#         if (idx == 0):
#             imageR = cv2.circle(imageR, tuple(pick[idx][i]), 10, red, 4)
#             imageR = cv2.putText(imageR, str(i), (pick[idx][i][0]+10, pick[idx][i][1]-10),
#                               cv2.FONT_HERSHEY_SIMPLEX,1, red, 1)

#         if (idx == 1):
#             imageC = cv2.circle(imageC, tuple(pick[idx][i]), 10, red, 4)
#             imageC = cv2.putText(imageC, str(i), (pick[idx][i][0]+10, pick[idx][i][1]-10),
#                               cv2.FONT_HERSHEY_SIMPLEX,1, red, 1)
